...
class ExecutionTimeProbe(AbstractProbe):
    """
    ...
    def stop(self, **kwargs):
        ...
        """
        if 'time_results' not in kwargs:
            log_print('Unable to get time specific results from test: no such property "time_results"', color='red')
            return

        super().stop()

        self.results[self.version] += kwargs.get('time_results')

        self.seconds = 'seconds' in kwargs
    ...
    def is_passed(self, **kwargs):
        ...
        for version in self.results:
            ...
            self.result_message = 'Time run difference: {:.2f}, {} (base) - {} sec, {} - {} sec'.format(
                time_diff,
                self.base_version,
                int(base_version_result / 1000) if not self.seconds else base_version_result,  # milliseconds to seconds
                version,
                int(comparable_version_result / 1000) if not self.seconds else comparable_version_result
